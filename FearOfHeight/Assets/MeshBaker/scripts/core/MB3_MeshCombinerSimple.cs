    ...
    public class MB3_MeshCombinerSingle : MB3_MeshCombiner
    {
		...
		*/
        ...
        public override GameObject resultSceneObject
        {
            set
            {
                if (_resultSceneObject != value)
                {
                    ...
                    {
                        Debug.LogWarning("Evaluation Scene Object was changed when this mesh baker component had a reference to a mesh. If mesh is being used by another object make sure to reset the mesh to none before baking to avoid overwriting the other mesh.");
                    }
                }
                          ...
                          GenerateUV2Delegate uv2GenerationMethod = null)
        {
            ...
            if (_mesh != null)
            {
                ...
                if (triangles && _textureBakeResults)
                {
                    ...
                    {
                        Debug.LogError("Material Bake Evaluation was not set.");
                    }
                    else {
                        int[][] submeshTrisToUse = GetSubmeshTrisWithShowHideApplied();
        ...
        public bool ValidateTargRendererAndMeshAndResultSceneObj()
        {
            ...
            {
                if (_LOG_LEVEL >= MB2_LogLevel.error) Debug.LogError("Evaluation Scene Object was not set.");
                return false;
            }
            else {
                if (_targetRenderer == null)
                {
                    ...
                    {
                        if (_LOG_LEVEL >= MB2_LogLevel.error) Debug.LogError("Target Renderer game object is not a child of Evaluation Scene Object was not set.");
                        return false;
                    }
        ...
        public static Renderer BuildSceneHierarch(MB3_MeshCombinerSingle mom, GameObject root, Mesh m, bool createNewChild = false, GameObject[] objsToBeAdded = null)
        {
            ...
            if (!createNewChild)
            {
                if (mom.targetRenderer != null)
                {
                    ...
                    if (rs.Length > 1)
                    {
                        Debug.LogError("Evaluation Scene Object had multiple child objects with renderers attached. Only one allowed. Try using a game object with no children as the Evaluation Scene Object.");
                        return null;
                    }
                    if (rs.Length == 1)
                    {
                        ...
                        {
                            Debug.LogError("Target Renderer is not an immediate child of Evaluation Scene Object. Try using a game object with no children as the Evaluation Scene Object..");
                            return null;
                        }
                        mt = rs[0].transform;
                    }
