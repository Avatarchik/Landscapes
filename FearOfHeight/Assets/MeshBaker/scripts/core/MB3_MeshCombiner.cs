			...
			if (r != null){
				...
				for (int i = 0; i < gos.Length; i++){
					...
					for (int j = 0; j <oMats.Length; j++){
						...
						if (found == false){
							Debug.LogError (string.Format("Material Bake Evaluation is null and game object {0} in list of objects to add did not have a subset of the materials in on the first object. You need to bake textures or all objects must have a subset of materials on the first object.",i));
							return false;
						}
					}
				}
