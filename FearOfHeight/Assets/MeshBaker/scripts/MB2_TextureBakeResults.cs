    ...
    public class Material2AtlasRectangleMapper
    {
                                              ...
                                              MB2_LogLevel logLevel)
        {
			if (tbr.materialsAndUVRects.Length == 0 && tbr.materials.Length > 0) {
				errorMsg = "The 'Material Bake Evaluation' needs to be re-baked to be compatible with this version of Mesh Baker. Please re-bake using the MB3_TextureBaker.";
            ...
            if (idx == -1)
            {
                ...
                encapsulatingRect = new Rect();
                sourceMaterialTilingOut = new Rect();
                errorMsg = String.Format("Material {0} could not be found in the Material Bake Evaluation", mat.name);
                return false;
            }
