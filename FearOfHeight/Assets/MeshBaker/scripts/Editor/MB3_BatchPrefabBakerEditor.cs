...
public class MB3_BatchPrefabBakerEditor : Editor {
	...
	public override void OnInspectorGUI(){
		...
		EditorGUILayout.HelpBox(
								...
								"4) choose a folder where the result prefabs will be stored and click 'Create Empty Evaluation Prefabs'\n" +
        ...
        if (GUILayout.Button("Create Empty Evaluation Prefabs"))
        {
	...
	public void _bakePrefabs(){
		...
		for (int i = 0; i < pb.prefabRows.Length; i++){
			...
			if (pb.prefabRows[i].resultPrefab == null){
				Debug.LogError("Evaluation Prefab on row " + i + " is not set.");
				return;
			}
