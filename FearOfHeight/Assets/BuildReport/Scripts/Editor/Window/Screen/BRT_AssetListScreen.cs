...
public class AssetList : BaseScreen
{
	...
	void InitiateDeleteSelectedInAssetList(BuildInfo buildReportToDisplay, BuildReportTool.AssetList listToDeleteFrom)
	{
		...
		if (numOfFilesToDelete <= 0)
		{
			if (brtFilesSelectedForDelete > 0)
			{
				...
				EditorUtility.DisplayDialog("Can't delete!", "Take note that for safety, Build Result Tool assets themselves will not be included for deletion.", "OK");
			}
			return;
		}
		...
		if (brtFilesSelectedForDelete > 0)
		{
			message += "\n\nTake note that for safety, " + brtFilesSelectedForDelete + " file" + ((brtFilesSelectedForDelete > 1) ? "s" : "") + " found to be Build Result Tool assets are not included for deletion.";
		}
	...
	void InitiateDeleteAllUnused(BuildInfo buildReportToDisplay)
	{
		...
		{
			const string nothingToDelete = "Take note that for safety, Build Result Tool assets, Unity editor assets, version control metadata, and Unix-style hidden files will not be included for deletion.\n\nYou can force deleting them by selecting them (via the checkbox) and using \"Delete selected\", or simply delete them the normal way in your project view.";
		...
		if (!EditorUtility.DisplayDialog("Delete?",
				"Among " + all.Length + " file" + plural + " in your project, " + filesToDeleteCount + " will be deleted.\n\nBuild Result Tool assets themselves, Unity editor assets, version control metadata, and Unix-style hidden files will not be included for deletion. You can force-delete those by selecting them (via the checkbox) and use \"Delete selected\", or simply delete them the normal way in your project view.\n\nDeleting a large number of files may take a long time as Unity will rebuild the project's Library folder.\n\nAre you sure about this?\n\nThe file" + plural + " can be recovered from your " + BuildReportTool.Util.NameOfOSTrashFolder + ".", "Yes", "No"))
		{
			return;
		}
