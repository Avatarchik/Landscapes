...
namespace EnhancedHierarchy {
    ...
    internal static class GUIDrawer {
        ...
        private static void ItemGUI(int instanceID, Rect rect) {
            try {
                ...
                if(Prefs.colorSeparator && isFirstVisible)
                    ColorSort(rect);
                if(Prefs.tree)
                    DrawTree(rect);
                if(Prefs.warning)
                    DrawWarnings(rect);
                else if(Prefs.trailing)
                    DoTrailing(rect);
                ...
                //if(isFirstVisible)
                //    Utility.ShowFPS(evt, new Rect(Vector2.zero, new Vector2(65f, 16f)));
            }
            catch(Exception e) {
        ...
        private static void DrawPrefabApply(Rect rect) {
            ...
            if(GUI.Button(rect, Styles.prefabApplyContent, Styles.applyPrefabStyle))
                if(isPrefab) {
                    ...
                    var path = EditorUtility.SaveFilePanelInProject("Save prefab", "newPrefab", "prefab", "Save the selected prefab");
                    if(path.Length > 0)
        ...
        private static void DrawWarnings(Rect rect) {
            ...
            for(int i = 0; i < contextEntries.Length; i++) {
                if(!hasInfo && contextEntries[i].Value == (contextEntries[i].Value | EntryMode.ScriptingLog))
                    hasInfo = true;
                if(!hasWarning && contextEntries[i].Value == (contextEntries[i].Value | EntryMode.ScriptingWarning))
                    hasWarning = true;
                if(!hasError && contextEntries[i].Value == (contextEntries[i].Value | EntryMode.ScriptingError))
                    hasError = true;
            }
        ...
        private static void DoTrailing(Rect rect, int icons = 0) {
    ...
    internal static class Prefs {
        ...
        static Prefs() {
            ...
            trailingContent = new GUIContent("Trailing", "Add ... when names are bigger than the view area");
    ...
    internal static class Utility {
        ...
        public static void ShowFPS(Event evt, Rect rect) {
    ...
    internal struct LogEntry {
        ...
        public static Dictionary<Object, EntryMode> referencedObjects { get; private set; }
        ...
        static LogEntry() {
            try {
                ...
                if(Prefs.warning)
                    Debug.LogWarningFormat("Enhanced Hierarchy, error getting console entries, if the error persists, consider disabling \"warning\" in the preferences: {0}", e);
            }
        ...
        private static void ReloadReferences() {
            try {
                referencedObjects = new Dictionary<Object, EntryMode>();
